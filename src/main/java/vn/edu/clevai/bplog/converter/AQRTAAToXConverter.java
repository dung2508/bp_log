...
@Component
@Slf4j
public class AQRTAAToXConverter extends BaseXConverter {
	private final BpULCService ulcService;

	public AQRTAAToXConverter(
	...
	public void execute(String xdsc, String lct, String dfge, List<BpUniqueLearningComponent> ulcs) {
		BpUniqueLearningComponent aqrtAa = ulcs.stream().findFirst().orElseThrow(
				() -> new NotFoundException("No ulc to be converted")
		);

		BpUniqueLearningComponent hrtgEa = ulcService.findByCode(aqrtAa.getMyParent());

		Set<String> xclasses = getBpClagClassgroupService()
				.findClagFromULC(hrtgEa.getMyParent())
				.stream()
		...
		if (xclasses.isEmpty()) {
			throw new NotFoundException("No valid xclass found for the shift = " + hrtgEa.getMyParent());
		}
		...
		List<CreateOrUpdateDscClassLearningObjectRequest> requests = loId2CodeMap
				...
				.map(
						e -> {
							...
							Integer group = bl42aqrt.get(code).get(0).getGroup();
