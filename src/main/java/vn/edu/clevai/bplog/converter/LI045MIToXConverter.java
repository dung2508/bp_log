...
@Component
@Slf4j
public class LI045MIToXConverter extends BaseXConverter {

	public LI045MIToXConverter(BpClagClassgroupService bpClagClassgroupService, AuthoringService authoringService, LmsService lmsService, UserService userService, ContentItemService contentItemService, CuiService cuiService) {
	...
	public void execute(String xdsc, String lct, String dfge, List<BpUniqueLearningComponent> ulcs) {
		...
		if (CollectionUtils.isEmpty(ulcs) || ulcs.size() > 1) {
			throw new BadRequestException("ulcs size must be 1 for LI045MIToXConverter!!!");
		}
